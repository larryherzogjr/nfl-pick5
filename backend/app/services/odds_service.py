...
ODDS_API_ROOT = "https://api.the-odds-api.com/v4/sports"
REQUEST_TIMEOUT_SECONDS = 30
NFL_GAME_TZ = ZoneInfo("America/New_York")
SCHEDULED_LOOKAHEAD_DAYS = 7


def _odds_api_url(phase: str) -> str:
    return f"{ODDS_API_ROOT}/{sport_key_for_phase(phase)}/odds"
...
def _find_week_for_kickoff(kickoff: datetime, phase: str) -> Week | None:
    kickoff_date = kickoff.astimezone(NFL_GAME_TZ).date()
    return (
        Week.query.filter(
            Week.phase == phase,
            Week.start_date <= kickoff_date,
            Week.end_date >= kickoff_date,
        )
...
def _scheduled_phases() -> list[str]:
    """Return phases whose active-season weeks need odds within seven days."""
    today = datetime.now(NFL_GAME_TZ).date()
    through = today + timedelta(days=SCHEDULED_LOOKAHEAD_DAYS)
    rows = (
...
def refresh_odds(week_id: int | None = None) -> dict:
    ...
    if week_id is not None:
        target_week = db.session.get(Week, week_id)
        if target_week is None:
            raise ValueError(f"Week {week_id} does not exist")
        phases = [target_week.phase]
    else:
        phases = _scheduled_phases()
    ...
    for phase in phases:
        ...
        for event in response.json():
            ...
            week = _find_week_for_kickoff(kickoff, phase)
            if week is None:
                logger.info(
                    "Skipping event %s (%s @ %s, kickoff %s): "
                    "no %s week matches kickoff date",
                    external_id,
                    away_abbr,
                    home_abbr,
                    kickoff.isoformat(),
            ...
            if selected is None:
                logger.info(
                    "Skipping event %s: no bookmaker offered spreads", external_id
                )
